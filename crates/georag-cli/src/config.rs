//! Configuration file support and loading utilities
...
impl ConfigFile {
    ...
    /// Get DATABASE_URL from configuration
    pub fn get_database_url(&self) -> Option<String> {
        self.postgres.as_ref().map(|pg| {
            if let Some(ref password) = pg.password {
                format!(
                    "postgresql://{}:{}@{}:{}/{}",
                    pg.user, password, pg.host, pg.port, pg.database
                )
            } else {
                format!(
                    "postgresql://{}@{}:{}/{}",
    ...
    pub fn create_default(workspace_dir: &Path, use_postgres: bool, database_url: Option<String>) -> Result<Self> {
    ...
    fn parse_database_url(url: &str) -> Result<PostgresConfig> {
        ...
        let (auth, rest) = url.split_once('@')
            .context("Invalid DATABASE_URL: missing @")?;
