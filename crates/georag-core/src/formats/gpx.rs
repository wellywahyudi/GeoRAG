//! GPX (GPS Exchange Format) reader implementation
//!
//! This module provides support for reading GPX files, which are XML-based
//! formats for GPS data including tracks, routes, and waypoints.
...
impl FormatReader for GpxReader {
    async fn read(&self, path: &Path) -> Result<FormatDataset> {
        ...
        // Extract features from tracks, routes, and waypoints
        ...
        Ok(FormatDataset {
            ...
            crs: 4326, // GPX always uses WGS84 (EPSG:4326)
            features,
        })
    }
        ...
        for (track_idx, track) in gpx.tracks.iter().enumerate() {
            ...
            for (seg_idx, segment) in track.segments.iter().enumerate() {
                ...
                let coordinates: Vec<serde_json::Value> = if has_elevation {
                    ...
                    segment.points.iter().map(|point| {
                        serde_json::json!([point.point().x(), point.point().y()])
                    }).collect()
                };
