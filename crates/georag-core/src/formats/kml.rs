...
impl KmlReader {
    ...
    async fn read_internal(
        ...
        let kml: Kml = content.parse().map_err(|e| GeoragError::FormatValidation {
            format: "KML".to_string(),
        ...
        Ok(FormatDataset {
            ...
            format_metadata: FormatMetadata {
                ...
                layer_name: folder_filter.map(|s| s.to_string()),
                page_count: None,
    ...
    fn convert_geometry(&self, geometry: &kml::types::Geometry) -> Result<serde_json::Value> {
        match geometry {
            kml::types::Geometry::Point(point) => Ok(self.convert_point(point)),
            kml::types::Geometry::LineString(linestring) => Ok(self.convert_linestring(linestring)),
            kml::types::Geometry::LinearRing(ring) => Ok(self.convert_linear_ring(ring)),
            kml::types::Geometry::Polygon(polygon) => Ok(self.convert_polygon(polygon)),
            kml::types::Geometry::MultiGeometry(multi) => Ok(self.convert_multi_geometry(multi)?),
            _ => Err(GeoragError::FormatError {
                format: "KML".to_string(),
    ...
    fn convert_linestring(&self, linestring: &kml::types::LineString) -> serde_json::Value {
        ...
        let coordinates: Vec<serde_json::Value> = if has_altitude {
            ...
            linestring
                .coords
                .iter()
                .map(|coord| serde_json::json!([coord.x, coord.y]))
                .collect()
        };
    ...
    fn convert_linear_ring(&self, ring: &kml::types::LinearRing) -> serde_json::Value {
        ...
        let coordinates: Vec<serde_json::Value> = if has_altitude {
            ...
            ring.coords.iter().map(|coord| serde_json::json!([coord.x, coord.y])).collect()
        };
    ...
    fn convert_ring_coords(&self, coords: &[kml::types::Coord]) -> Vec<serde_json::Value> {
        ...
        if has_altitude {
            ...
            coords.iter().map(|coord| serde_json::json!([coord.x, coord.y])).collect()
        }
    }
